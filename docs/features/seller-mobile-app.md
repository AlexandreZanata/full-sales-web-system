...
13. **Settings + media (16E / OD-16-8=A):** Room `site_settings` + `media_url_cache` (fileId, url, expiresAt). Offline image resolve uses non-expired cache; expired/missing → null/placeholder (no crash). Sync best-effort `pullSettings()`. **OD-16-6=B:** top-seller chips hide when offline.

**Manual device script:** sync catalog online → open product with image → airplane → image still loads if URL unexpired → create sale offline (no top-seller chips) → confirm offline (chained) → submit registration offline → go Online → outbox drains.
