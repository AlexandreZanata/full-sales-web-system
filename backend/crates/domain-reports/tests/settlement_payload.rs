...
use domain_reports::{
    PAYLOAD_VERSION, ReportError, SETTLEMENT_DISCLAIMER, is_eligible_for_report,
    sign_canonical_payload, verify_canonical_payload,
};
...
fn given_settled_sales_when_assembled_then_golden_canonical_json() {
    let payload = assembly_with_sales(vec![delivered_sale_fact(DeclaredPaymentMethod::Pix, true)])
        .assemble()
        .expect("assemble");
...
fn given_declared_settlement_when_assembled_then_disclaimer_present() {
    let payload = assembly_with_sales(vec![delivered_sale_fact(DeclaredPaymentMethod::Cash, true)])
        .assemble()
        .expect("assemble");
...
fn given_mixed_declarations_when_assembled_then_totals_by_method() {
    ...
    pix_sale.sale_id =
        SaleId::from_uuid(Uuid::parse_str("0190c001-1111-2222-3333-444455556666").expect("sale"));
    pix_sale.amount_cents = 30_000;

    let mut cash_sale = delivered_sale_fact(DeclaredPaymentMethod::Cash, true);
    cash_sale.sale_id =
        SaleId::from_uuid(Uuid::parse_str("0190c002-2222-3333-4444-555566667777").expect("sale"));
    ...
    assert!(
        payload
            .canonical_json
            .contains(r#""totalDeclaredCents":45000"#)
    );
}
