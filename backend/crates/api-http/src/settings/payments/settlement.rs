...
use super::support::{ensure_admin, enforce_settlement_rate_limit, map_billing_api, tenant_asaas_client};
use super::types::{
...
pub async fn get_payment_balance(
    ...
    if let Some(cached) = state.settlement_cache.get(&cache_key).await {
        return Ok(Json(serde_json::from_value(cached).map_err(|_| ApiError::internal())?));
    }
    ...
    state
        .settlement_cache
        .set(cache_key, serde_json::to_value(&response).map_err(|_| ApiError::internal())?)
        .await;
    Ok(Json(response))
}
