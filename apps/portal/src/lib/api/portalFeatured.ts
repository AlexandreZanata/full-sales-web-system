...
function featuredProductsPath(limit: number): { path: string; init?: { skipAuth: true } } {
  const query = new URLSearchParams({ limit: String(limit) });
  const hasSession = Boolean(getAccessToken());
  return hasSession
    ? { path: `/portal/products/featured?${query}` }
    : { path: `/public/products/featured?${query}`, init: { skipAuth: true } };
}

export async function fetchPortalFeaturedProducts(limit = 12): Promise<PortalProduct[]> {
  try {
    const { path, init } = featuredProductsPath(limit);
